...
<body>
	...
	<section class="container">
		...
		<% 
            ...
            ResultSet resultset = statement.executeQuery("SELECT * FROM tbl_user_fund_info WHERE Status_Info='" + str + "' AND due_date > CURDATE()");
        ...
        %>
           <TR>
               ...
               <TD><a href="User_Fund_Data.jsp?id=<%=resultset.getString(1)%>&U_Name=<%=resultset.getString(2)%>&U_Email_ID=<%=resultset.getString(3)%>&U_Fund_name=<%=resultset.getString(4) %>&U_Fund_cost=<%=resultset.getString(5)%> &U_Total_Fund=<%=resultset.getString(7)%> "><input type="button" name="sendkey" value="Next" class="btn btn-primary"></a></TD>
            </TR>
       <% 
           } 
       %>
